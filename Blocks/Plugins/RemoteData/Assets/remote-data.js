(function($) {
  var Field = acf.Field.extend({
     ...
     */
    stickyItems() {
      return this.empty(this.$stickyInput().val())
        ? []
        : this.$stickyInput()
            .val()
            .split(",");
    ...
    initialize() {
      ...
      this.set(
        "canSticky",
        parseInt(this.$control().get(0).dataset.can_sticky)
      );
      ...
      const delayed = this.proxy(
        acf.once(() => {
          ...
          this.$taxonomyRow()
            .not(":first")
            .each(index =>
              this.initializeTaxonomyFilters(
                $(this.$taxonomyRow().get(index + 1))
              )
            );
    ...
    onChangeFilter(e, $el) {
      ...
      if (this.empty(val)) {
        if (filter == "s") this.onClickClear();
        return;
      }

      if (this.get(filter) == val) return;
    ...
    onClickSticky(e, $el) {
      ...
      if (sticky) {
        ...
        if (e.type === "click")
          this.$isExceeded(
            this.stickyItems().length + 1 >= parseInt(this.$limitInput().val())
          );
        ...
        if ($li[0].hasAttribute("data-manual")) {
          ...
          this.$manualInput()
            .val(JSON.stringify(manualFilterId))
            .trigger("change");
        }

        this.$choicesList()
          .find(`[data-id="${$li.data("id")}"]`)
    ...
    getAjaxData() {
      ...
      for (let name in ajaxData) ajaxData[name] = this.get(name);
    ...
    fetch() {
      ...
      // Loading
      const $loading = $(
        `<li class="-loading"><i class="acf-loading"></i>${acf.__(
          "Loading"
        )}</li>`
      );
      $list.append($loading);
      ...
      const onComplete = () => {
        ...
        let hasManualData =
          this.$manualInput().val() !== ""
            ? JSON.parse(this.$manualInput().val())
            : [];
        // add edit button to manual lists
        if (hasManualData.length)
          this.$stickyList()
            .find("li[data-manual] > .acf-rel-item")
            .append(
              '<button class="editManualButton acf-js-tooltip" type="button" data-action="edit-manual" aria-label="Editar" title="Editar"><svg viewBox="0 0 24 24" width="10" height="10" stroke="currentColor" aria-label="hidden" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg></button>'
            );
      };
    ...
    getSearchData() {
      ...
      this.$valuesList()
        .find("li")
        .each((_, element) => ajaxData.exclude.push(element.dataset.id));
    ...
    search() {
      ...
      const onSuccess = json => {
        ...
        if (!json || !json.results || !json.results.length)
          // Add message
          return this.$choicesList().append(
    ...
    walkChoices(data, sticky = true) {
      ...
      const stickyManual = this.$manualInput().val().length
        ? JSON.parse(this.$manualInput().val())
        : [];
      ...
      let mergedData = stickyOrder.length
        ? [].concat(stickyOrder, mergeItems)
        : mergeItems;

      mergedData.forEach(element => {
        // results has empty
        if (Object.values(element).includes("0"))
          return this.$valuesList().append(
            `<li>${acf.__("No matches found")}</li>`
          );

        // html
        let content = `<li data-id="${acf.escAttr(
          element.id
        )}" data-date="${acf.escAttr(element.date)}" title="${acf.escAttr(
          element.title.rendered
        )}"`;
        content += `${
          element.id.toString().startsWith("m") ? " data-manual" : ""
        }><span class="acf-rel-item">`;

        if (
          (sticky && !this.empty(this.get("canSticky"))) ||
          (sticky &&
            this.empty(this.get("canSticky")) &&
            stickyItems.includes(element.id.toString()))
        )
          content +=
        ...
        if (element.hasOwnProperty("featured_media_url")) {
          if (element.featured_media_url.hasOwnProperty("pa-block-preview"))
            content += this.empty(
              element.featured_media_url["pa-block-preview"]
            )
              ? '<div class="thumb"></div>'
              : `<img src="${element.featured_media_url["pa-block-preview"]}" alt="Thumbnail" />`;
          else if (element.featured_media_url.hasOwnProperty("pa_block_render"))
            content += this.empty(element.featured_media_url["pa_block_render"])
              ? '<div class="thumb"></div>'
              : `<img src="${element.featured_media_url["pa_block_render"]}" alt="Thumbnail" />`;
          else if (element.featured_media_url.hasOwnProperty("pa-block-render"))
            content += this.empty(element.featured_media_url["pa-block-render"])
              ? '<div class="thumb"></div>'
              : `<img src="${element.featured_media_url["pa-block-render"]}" alt="Thumbnail" />`;
        } else content += '<div class="thumb"></div>';
        ...
        if (stickyItems.includes(element.id.toString())) stickyList += content;
        else list += content;
      });
      ...
      this.$limitInput().change(e =>
        this.$isExceeded(stickyItems.length >= e.target.value)
      );
    ...
    onClickAdd(e, $el) {
      ...
      if (this.stickyItems().length == limit) {
        // Add notice
        this.showNotice({
          text: `Limite máximo de ${limit} ite${
            limit == 1 ? "m" : "ns"
          } atingido`,
          type: "warning"
        });
    ...
    sortValues() {
      ...
      this.$stickyList()
        ...
        .each((_, element) =>
          this.$stickyInput().val(
            `${this.$stickyInput().val()},${element.dataset.id}`
          )
        );
    ...
    modalContent($modal, data = null) {
      ...
      const $loading = $(
        `<span class="-loading"><i class="acf-loading"></i>${acf.__(
          "Loading"
        )}</span>`
      );
      ...
      const onSuccess = d => {
        ...
        $modalContent
          .find(
            '[data-name="link"] [data-name="add"], [data-name="link"] [data-name="edit"]'
          )
          .on("click", () => $("#wp-link-wrap").addClass("-no-label"));
        $(
          "#wp-link-close, #wp-link-submit, #wp-link-cancel button, #wp-link-backdrop"
        ).on("click", () => $("#wp-link-wrap").removeClass("-no-label"));
      };
    ...
    parseModalFields($modal, id = null) {
      ...
      $fields.each((_, element) => {
        ...
        else values[field.data.name] = field.getValue();
      });
      ...
      if (!id) id = `m${$date.getUTCMilliseconds()}`;
    ...
    onClickAddManualPost() {
      ...
      this.modal.open($modal, {
        ...
        onOpen: () => {
          ...
          modalHeader.append(
            '<button class="button button-primary button-sticky-add" data-name="manualSubmit">Adicionar</button>'
          );
    ...
    onEditManual(e, $el) {
      ...
      this.modal.open($modal, {
        ...
        onOpen: () => {
          ...
          let modalHeader = $modal.find(".widgets-acf-modal-title");
          modalHeader.append(
            '<button class="button button-primary button-sticky-add" data-name="editSubmit">Conlcuir</button>'
          );
          ...
          buttonEdit.click(() => {
            ...
            const objectsEqual = (oldValue, newValue) => {
              return typeof oldValue === "object" &&
                ...
                ? Object.keys(oldValue).length ===
                    ...
                    Object.keys(oldValue).every(p =>
                      objectsEqual(oldValue[p], newValue[p])
                    )
                : oldValue === newValue;
            };
            ...
            if (!compare) {
              // append updated values to input
              this.$manualInput()
                .val(JSON.stringify(updatedData))
                .trigger("change");
              this.fetch();
              this.modal.close();
            }
    ...
    onClickAddTaxonomy(e, $el) {
      if (
        ...
        this.$taxonomyRow().not(":first").length
      )
        return;
        ...
        this.$taxonomyRow()
          ...
          .each(index => {
            ...
            if ($selectTaxonomy.length)
              $selectTaxonomy.attr(
                "name",
                `acf[${this.get("key")}][taxonomies][${index}]`
              );
            if ($selectTerms.length)
              $selectTerms.attr(
                "name",
                `acf[${this.get("key")}][terms][${index}][]`
              );
          });
    ...
    initializeTaxonomyFilters($row, isNew = false) {
      ...
      if (!$selectTaxonomy.length) return;

      $selectTaxonomy.attr(
        "name",
        `acf[${this.get("key")}][taxonomies][${this.$taxonomyRow().length - 2}]`
      );
      $selectTerms.attr(
        "name",
        `acf[${this.get("key")}][terms][${this.$taxonomyRow().length - 2}][]`
      );

      if (isNew) {
        const $selects = this.$taxonomyRow()
          .not(":first")
          .not(":last")
          .find("[data-taxonomy]");
        let values = [];
      ...
      $selectTaxonomy.on("change", () => {
        ...
        $selectTaxonomy
          .find(`option[value="${$selectTaxonomy.val()}"]`)
          .attr("selected", true);
      ...
      if (isNew) $selectTaxonomy.trigger("change");
    ...
    checkTaxonomyFilters() {
      if (!this.$buttonAddTaxonomy().length) return;
      // Enable/disable button
      this.$buttonAddTaxonomy().toggleClass(
        "disabled",
        Object.keys(this.taxonomies()).length ==
          this.$taxonomyRow().not(":first").length
      );
    ...
    saveTaxonomyFilters() {
      ...
      // Get selected values
      $selectTaxonomy.each(index =>
        taxonomies.push($($selectTaxonomy.get(index)).val())
      );
    ...
    modal: {
      ...
      open: ($target, args) => {
        ...
        if (
          !$target.find(
            "> .widgets-acf-modal-wrapper > .widgets-acf-modal-content"
          ).length
        )
          $target
            .find("> .widgets-acf-modal-wrapper")
            .wrapInner('<div class="widgets-acf-modal-content" />');

        $target
          .find("> .widgets-acf-modal-wrapper")
          .prepend(
            '<div class="widgets-acf-modal-wrapper-overlay"></div><div class="widgets-acf-modal-title"><span class="title">' +
              args.title +
              '</span><button class="button button-secondary button-close">Cancelar</button></div>'
          );
      ...
      close: args => {
        ...
        if (modal.current) {
          ...
          if (args.destroy) modal.current.remove();
        }
      ...
      onOpen: ($target, args) => {
        if (!args.onOpen || !(args.onOpen instanceof Function)) return;

        args.onOpen($target);
      },

      onClose: ($target, args) => {
        if (!args.onClose || !(args.onClose instanceof Function)) return;
