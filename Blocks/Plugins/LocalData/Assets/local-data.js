(function($) {
  var Field = acf.Field.extend({
    ...
    getAjaxData() {
      ...
      ajaxData.limit =
        this.get("limit") < this.stickyItems().length
          ? this.stickyItems().length
          : this.get("limit");
    ...
    walkChoices(data, sticky = true) {
      ...
      mergedData.forEach(element => {
        ...
        content += `${
          element.id.toString().startsWith("m")
            ? ""
            : `<div class="badge__pill">${element.cpt_label.rendered}</div>`
        }`;
        content += `</div>`;
