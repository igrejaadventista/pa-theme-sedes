<?php if(isset($post) && !empty($post)): ?>
  <div class="pa-blog-itens mb-5">
    ...
    <div class="pa-blog-feature">
      <a href="<?= get_the_permalink($post->ID) ?>" title="<?php get_the_title($post->ID) ?>">
        <div class="ratio ratio-16x9">
          <figure class="figure m-xl-0 w-100">
            <img src="<?= check_immg($post->ID, 'full') ?>" class="figure-img img-fluid m-0 rounded w-100 h-100" alt="<?= get_the_title($post->ID) ?>">
            ...
            <figcaption class="figure-caption position-absolute w-100 p-3 rounded-bottom">
              ...
              <h3 class="h5 pt-2 pa-truncate-2"><?= get_the_title($post->ID) ?></h3>
            </figcaption>
          </figure>
        </div>
      </a>
