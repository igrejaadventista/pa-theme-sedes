      ...
      <div class="row">
          ...
          <div class="col-5 col-md-4">
            <div class="ratio ratio-16x9">
              <figure class="figure m-xl-0">
                <img src="<?= check_immg($post->ID, 'medium') ?>" class="figure-img img-fluid rounded m-0 h-100 w-100" alt="<?= get_the_title($post->ID) ?>">

                <?php if(isset($category) && !empty($category)): ?>
                  <figcaption class="pa-img-tag figure-caption text-uppercase rounded-right d-none d-md-table-cell"><?= $category ?></figcaption>
                <?php endif; ?>
              </figure>	
            </div>
          </div>
        ...
        <div class="col">
          <div class="card-body <?= has_post_thumbnail($post) ? 'p-0' : 'ps-4 pe-0 py-4 border-start border-5 pa-border' ?>">
            <?php if(isset($format) && !empty($format)): ?>
            ...
            <p class="d-none d-xl-block"><?= wp_trim_words(get_the_excerpt($post->ID), 30) ?></p>
          </div>
        </div>
      </div>
    </a>
